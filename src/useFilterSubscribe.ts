...
export const useFilterSubscribe = (params: UseFilterSubscribeParams): UseFilterSubscribeResult => {
    const {
        waku,
        decoder,
    } = params;
    ...
    const pushMessage = useCallback((message: IDecodedMessage): void => {
    ...
    useEffect(() => {
        ...
        waku?.filter?.subscribe([decoder], pushMessage)
            .then((unsubscribeFn) => {
                setLoading(false);
            ...
            .catch((err) => {
                setLoading(false);
                setError(`Failed to subscribe to filer: ${err?.message || "no message"}`);
            });
